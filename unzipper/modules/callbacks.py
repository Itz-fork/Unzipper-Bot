...
async def unzipper_cb(_, query: CallbackQuery):
    ...
    if qdat == "megoinhome":
        await query.edit_message_text((await unzipperbot.get_string("start")).format(query.from_user.mention), reply_markup=Buttons.START_BUTTON)
        ...
        await query.edit_message_text((await unzipperbot.get_string("about")).format(unzipperbot.nx_version), reply_markup=Buttons.BACK, disable_web_page_preview=True)

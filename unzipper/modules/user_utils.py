...
async def start_bot(_, message: Message):
    await message.reply_text((await unzip_client.get_string_key("start")).format(message.from_user.mention), reply_markup=Buttons.START, disable_web_page_preview=True)
...
async def give_my_thumb(_, message: Message):
    ...
    if not gthumb:
        return await prs_msg.edit(await unzip_client.get_string_key("no_thumb"))
...
async def do_backup_files(_, message: Message):
    prs_msg = await message.reply(await unzip_client.get_string_key("processing"), reply_to_message_id=message.id)
    await prs_msg.edit(await unzip_client.get_string_key("select_provider"), reply_markup=Buttons.BACKUP)


@unzip_client.on_message(filters.private & filters.command("clean"))
async def clean_ma_files(_, message: Message):
    prs_msg = await message.reply(await unzip_client.get_string_key("processing"), reply_to_message_id=message.id)
    await prs_msg.edit(await unzip_client.get_string_key("ask_clean"), reply_markup=Buttons.CLEAN)
